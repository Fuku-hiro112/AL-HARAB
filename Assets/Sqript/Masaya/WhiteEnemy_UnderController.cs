...
public class WhiteEnemy_UnderController : MonoBehaviour
{
    ...
    public float detectionDistance = 10f;  // 敵が移動を開始するプレイヤーとの距離
    ...
    private void Update()
    {
        ...
        if (Vector3.Distance(transform.position, playerTransform.position) <= detectionDistance)
        {
            ...
            {
                isMoving = true;
                targetPosition = new Vector3(transform.position.x, transform.position.y + 3f, transform.position.z);  // 上に移動する位置を設定
            }
        }
